...
def legislators(request):
    ...
    for legislator in legislators:
        ...
        votes_in_favor = VoteResult.objects.filter(
            legislator_id=legislator.id, vote_type=1
        ).count()
        votes_against = VoteResult.objects.filter(
            legislator_id=legislator.id, vote_type=2
        ).count()
    ...
    return render(
        request, "core/legislators.html", {"legislator_data": legislator_data}
    )
...
def bills(request):
    ...
    for bill in bills:
        ...
        if vote:
            votes_in_favor = VoteResult.objects.filter(
                vote_id=vote.id, vote_type=1
            ).count()
            votes_against = VoteResult.objects.filter(
                vote_id=vote.id, vote_type=2
            ).count()
